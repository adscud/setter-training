import { TrainingContext } from "@/contexts/training"
import { Link } from "@tanstack/react-router"
import { useContext } from "react"
import { Detection } from "../features/detection/Detection"

export function Training() {
	const { sets } = useContext(TrainingContext)

	return (
		<>
			{sets.length > 1 ? (
				<Detection />
			) : (
				<div className="h-full w-full flex p-4">
					<section className="p-4 m-auto bg-white flex flex-col items-center rounded-lg">
						<div className="flex items-center rounded-lg">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
								strokeWidth="1.5"
								stroke="currentColor"
								className="w-6 h-6 mr-2 stroke-indigo-500"
							>
								<path
									strokeLinecap="round"
									strokeLinejoin="round"
									d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z"
								/>
							</svg>

							<p className="text-lg text-indigo-500">
								Please add at least 2 sets to start training.
							</p>
						</div>
						<Link to="/" className="text-indigo-500 underline">
							Back
						</Link>
					</section>
				</div>
			)}
			<section className="absolute z-10 top-4 left-4 flex">
				<Link
					to="/"
					className={`bg-transparent hover:bg-indigo-950 mr-4 px-4 py-2 flex justify-center items-center rounded-lg`}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
						strokeWidth="1.5"
						stroke="currentColor"
						className="w-6 h-6 stroke-white	"
					>
						<path
							strokeLinecap="round"
							strokeLinejoin="round"
							d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18"
						/>
					</svg>
				</Link>
				<p className="text-3xl font-bold text-white">Training</p>
			</section>
		</>
	)
}
